...
class FileFormat(IntEnum):
    """
    The :class:`~esmpy.api.grid.Grid` and :class:`~esmpy.api.mesh.Mesh` objects 
...
class GridItem(IntEnum):
    """
    The :class:`~esmpy.api.grid.Grid` can contain other kinds of data besides coordinates. This
    data is referred to as :class:`~esmpy.api.grid.Grid` "items". Some items may be used
    for calculations involving the :class:`~esmpy.api.grid.Grid`.
    """
    INVALID = -2
    UNINIT = -1
    MASK = 0
...
class MeshElemType(IntEnum):
    """
    A :class:`~esmpy.api.mesh.Mesh` can be constructed from a combination of different elements.
    The type of elements that can be used in a :class:`~esmpy.api.mesh.Mesh` depends on the
    parametric dimension of the :class:`~esmpy.api.mesh.Mesh`, which is set during :class:`~esmpy.api.mesh.Mesh`
    creation. The following are the valid :class:`~esmpy.api.mesh.Mesh` element types for each
    valid :class:`~esmpy.api.mesh.Mesh` parametric dimension (2D or 3D).
    """
    TRI = 3
    """
...
class MeshLoc(IntEnum):
    """
    The :class:`~esmpy.api.mesh.Mesh` location used to hold :class:`~esmpy.api.field.Field` data.
    """
    NODE = 0
    """
    The nodes of the :class:`~esmpy.api.mesh.Mesh`.
    """
    ELEMENT = 1
    """
    The elements of the :class:`~esmpy.api.mesh.Mesh`.
    """
...
class PoleMethod(IntEnum):
    ...
    :class:`~esmpy.api.grid.Grid` for regridding.
    """
    NONE = 0
    """
    No pole. Destination points which lie above the top or below the bottom row
    of the source :class:`~esmpy.api.grid.Grid` won't be mapped.
    """
    ALLAVG = 1
    """
    ...
    by constructing triangles across the top and bottom row of the source :class:`~esmpy.api.grid.Grid`.
    ...
    still not be able to be mapped to the source :class:`~esmpy.api.grid.Grid`.
    """
...
class Region(IntEnum):
    """
    Specify various regions in the data layout of a :class:`~esmpy.api.field.Field` object.
    """
    TOTAL = 0
    """
    ...
    to specify that the portions of a :class:`~esmpy.api.field.Field` that are not mapped in a regridding
...
class RegridMethod(IntEnum):
    ...
    provided in the :class:`~esmpy.api.grid.Grid`. It currently only works for 
    :class:`Fields <esmpy.api.field.Field>`s created on the 
...
class StaggerLoc(IntEnum):
    """
    In the :class:`~esmpy.api.grid.Grid` class, data can be located at different positions in a
    :class:`~esmpy.api.grid.Grid` cell. When setting or retrieving coordinate data the stagger
    location is specified to tell the :class:`~esmpy.api.grid.Grid` method from where in the
